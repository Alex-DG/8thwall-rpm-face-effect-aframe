...
const avatarFaceComponent = () => {
  const avatarAnchorComponent = {
    schema: {
      ...
      eyeR: { type: 'number', default: 0 }, // rightEye
      eyeL: { type: 'number', default: 0 }, // leftEye
    },
    init() {
      ...
      /**
       * Check: https://www.8thwall.com/docs/api/facecontroller/attachmentpoints/
       * and: https://docs.readyplayer.me/ready-player-me/api-reference/avatars/morph-targets/apple-arkit
       */
      // eye left
      this.eyeLeft = document.createElement('xrextras-face-attachment')
      this.eyeLeft.setAttribute('alignToSurface', 'true')
      this.eyeLeft.setAttribute('point', 'leftUpperEyelid')
      this.eyeLeft.id = 'eyeLeft'
      this.el.appendChild(this.eyeLeft)

      // eye right
      this.eyeRight = document.createElement('xrextras-face-attachment')
      this.eyeRight.setAttribute('alignToSurface', 'true')
      this.eyeRight.setAttribute('point', 'rightUpperEyelid')
      this.eyeRight.id = 'eyeRight'
      ...
      console.log({ el: this.el, elChildren: this.el.children[0] })
    },
    tick() {
      ...
      const mouthYDist =
        this.upper.object3D.position.y - this.lower.object3D.position.y

      const mouthXDist =
        this.mouthLeft.object3D.position.x - this.mouthRight.object3D.position.x

      const mouthLeftYDist =
        this.mouthLeft.object3D.position.y -
        this.mouthCenter.object3D.position.y
      ...
      this.data.eyebrowL = blendAmt(
        this.browLeft.object3D.position.y,
        0.14,
        0.19
      )

      this.data.eyeL = blendAmt(
        this.eyeLeft.object3D.position.y,
        eyeOptions.open_min,
        eyeOptions.open_max
      )
      this.data.eyeR = blendAmt(
        this.eyeRight.object3D.position.y,
        eyeOptions.open_min,
        eyeOptions.open_max
      )
      ...
      // eyes
      this.el.children[0].setAttribute('gltf-morph__eyeLeft', {
        morphtarget: 'eyeBlinkLeft',
        value: this.data.eyeL,
      })
      this.el.children[0].setAttribute('gltf-morph__eyeRight', {
        morphtarget: 'eyeBlinkRight',
        value: this.data.eyeR,
      })
    },
  }
