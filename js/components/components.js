  ...
  tick(time, timeDelta) {
    ...
    const inputCheck = (input) => {
      switch (input) {
        case 'xrGamepad':
          ...
          if (
            this.vrType === 'controllers' &&
            this.el.sceneEl.xrSession.inputSources.length > 0
          ) {
            const controllers = Array.from(
              this.el.sceneEl.xrSession.inputSources
            )
            let vrLeftVert
            let vrLeftHoriz
            ...
            for (let i = 0; i < controllers.length; i++) {
              if (controllers[i].handedness === 'left') {
                vrLeftVert =
                  this.el.sceneEl.xrSession.inputSources[i].gamepad.axes[3]
                vrLeftHoriz =
                  this.el.sceneEl.xrSession.inputSources[i].gamepad.axes[2]
              }
            }

            if (
              vrLeftVert > sensitivity ||
              vrLeftVert < -sensitivity ||
              vrLeftHoriz < -sensitivity ||
              vrLeftHoriz > sensitivity
            ) {
        ...
        case 'gamepad':
          ...
          if (this.gamepads[0]) {
            ...
            if (
              gamepadLeftVert > sensitivity ||
              gamepadLeftVert < -sensitivity ||
              gamepadLeftHoriz < -sensitivity ||
              gamepadLeftHoriz > sensitivity
            ) {
    ...
    if (startPositionRaw) {
      ...
      const maxRawDistance =
        Math.min(window.innerWidth, window.innerHeight) / (isDesktop ? 12 : 6.5)
    ...
    if (this.isMoving) {
      ...
      this.avatar.position.z -=
        this.speed * Math.sin(this.joystickRot) * timeDelta
      this.avatar.position.x -=
        this.speed * Math.cos(this.joystickRot) * timeDelta
